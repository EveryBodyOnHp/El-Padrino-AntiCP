local action = function(msg, blocks, ln) local hash = 'chat:'..msg.chat.id..':topic' local topic = db:get(hash) local texto = 
blocks[2] if not (msg.chat.type == 'private') then
...
texto`', true)
 ...
 end end -- cierra el if is mod
 else
 api.sendMessage(msg.from.id, make_text(lang[ln].pv)) end -- cierra el if not private end -- cierra la funcion/plugin return {
	action = action,
	triggers = {
			'^/(topic)$',
			'^/(topic)@'..bot.username..'$',
