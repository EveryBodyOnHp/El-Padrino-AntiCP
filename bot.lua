...
bot_init = function(on_reload)
	...
	print(colors.blue..'Leyendo lenguajes.lua...' ..colors.reset)
	...
	print(colors.blue..'Plugins totales leidos:', #plugins ..colors.reset)
			...
			if v.triggers then
				for k,w in pairs(v.triggers) do
					...
					if blocks then
						...
						if not success then
							api.sendReply(msg, '*Hubo un problema*\nPor favor, reportalo con `@HuterCR` :)', true)
